...
const _0x1366a8 = _0x1c156a("顺丰速运"),
  ...
  _0x94f81d = process.env[_0x3d0561 + "Bee"] || "true",//或 'false'，根据需要设置开启采蜜
  _0x3bec27 = process.env[_0x3d0561 + "DragonBoatDraw"] || "false",
  _0x4f8a2c = process.env[_0x3d0561 + "Wealth"] || "true",//或 'false'，根据需要设置开启财神
  _0x5b274f = 8000,
  _0x238e48 = 3;
...
class _0x520715 extends _0x49bd9d {
  ...
  getSign(_0x55a02d = {}) {
    let _0x3ad26b = Date.now(),
      _0x7c3a32 = "token=" + _0x55a02d + "&timestamp=" + _0x3ad26b + "&sysCode=" + _0x2bbd78,
  ...
  async jika2024_getAward(_0x42f65d, _0x14b34f = {}) {
    try {
      ...
      let _0x5dcdf7 = {
          ...
          method: "post",
          url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024GardenPartyService~getAward",
          headers: {
            ...this.getSign()
          },
          json: _0x31f302
  ...
  async jika2024_chengyu_index(_0x32d187 = {}) {
    try {
      let _0x3a5581 = {
          ...
          method: "post",
          url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024GuessIdiomService~index",
          headers: {
            ...this.getSign()
          },
          json: {}
  ...
  async jika2024_cardStatus(_0x372ae2 = {}) {
    try {
      let _0xd313b = {
          ...
          method: "post",
          url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024CardService~cardStatus",
          headers: {
            ...this.getSign()
          },
          json: {}
  ...
  async jika2024_collectDrawAward(_0xcaa39f = {}) {
    try {
      ...
      let _0x3fd1c4 = {
          ...
          method: "post",
          url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024CardService~collectDrawAward",
          headers: {
            ...this.getSign()
          },
          json: _0x2c6533
  ...
  async dragonBoat2024_game_indexInfo(_0x4ea6e4 = {}) {
    try {
        ...
        // 直接打印完整的请求信息
        this.log("indexInfo请求:", JSON.stringify(options, null, 2));
        
        let response = await this.request(options);
        // 打印原始response
        this.log("indexInfo完整返回:", JSON.stringify(response, null, 2));
        ...
        if(!response.result) {
            this.log("❌ 返回数据缺少result字段:", response);
            return null;
        }
        
        if(!response.result.success) {
            this.log("❌ 请求失败:", response.result.errorMessage || JSON.stringify(response.result));
            return null;
        }
        
        let gameInfo = response.result.obj;
        if(!gameInfo) {
            this.log("❌ 返回数据缺少obj字段");
            return null;
        }
        
        // 处理游戏状态
        return {
            surplusPushTime: gameInfo.surplusPushTime || 0,
            ifPassAllLevel: gameInfo.ifPassAllLevel,
            currentIndex: gameInfo.currentIndex,
            gotTimesOfToday: gameInfo.gotTimesOfToday
        };
        
    } catch(e) {
        this.log("indexInfo异常:", e);
        console.log(e);
    }
    return null;
  }
  ...
  async dragonBoat2024_game_init(_0x4390fe, _0x437d67 = {}) {
    try {
        ...
        // 直接打印完整的请求信息
        this.log("init请求:", JSON.stringify(options, null, 2));
        
        let response = await this.request(options);
        // 打印原始response
        this.log("init完整返回:", JSON.stringify(response, null, 2));
        ...
        return {
            currentIndex: gameInfo.currentIndex,
            resurrectTime: gameInfo.resurrectTime,
            currentLevelAward: gameInfo.currentLevelAward,
            levelConfig: gameInfo.levelConfig
        };
  ...
  async userTask(_0x358065 = {}) {
    ...
    // 等待获取账号信息
    await _0x1366a8.wait(1000);
    
    // 收集账号基本信息
    if(this.point) {
        let pointInfo = `积分: ${this.point}, 有${this.expirePoint}积分将在${this.expireTime}过期`;
        _0x1366a8.log(pointInfo);
        this.accountMsg.push(pointInfo);
    }
    if(this.honey) {
        let honeyInfo = `采蜜游戏丰蜜: ${this.honey}`;
        _0x1366a8.log(honeyInfo);
        this.accountMsg.push(honeyInfo);
    }
    ("");  // 空行分隔
    ...
    if(_0x4f8a2c == "true") {
        // 1. 先执行套财神游戏
        await this.jika2024_game_play();
        
        // 2. 执行招财神任务
        await this.jika2024_wealthStatus();
    } 
    
    // 收集优惠券信息
    let coupons = await this.coupon_list();
    if(coupons && coupons.length > 0) {
        let couponInfo = ["大额优惠券:"];
        for(let coupon of coupons) {
            couponInfo.push(`${coupon.name}, 过期时间: ${coupon.expireTime}`);
        }
        _0x1366a8.log(couponInfo.join('\n'));
        this.accountMsg.push(couponInfo.join('\n'));
    }

    // 将该账号的消息添加到全局推送列表
    _0x1366a8.pushMsgList.push(this.accountMsg.join('\n'));
  }

  // 获取套财神游戏信息
  async jika2024_bonus_getPopInfo() {
    try {
      let _0x3a5581 = {
        fn: "jika2024_bonus_getPopInfo",
        method: "post",
      ...
      let { result: _0x3dd9fc } = await this.request(_0x3a5581);
      if (_0x3dd9fc?.["success"]) {
        // 打印详细信息
        if(_0x3dd9fc.obj) {
          this.log("当前排名: 【" + _0x3dd9fc.obj.rankNo + "】");
          this.log("超越玩家比例: 【" + _0x3dd9fc.obj.rankPercent + "%】");
          if(_0x3dd9fc.obj.healthChance) {
            this.log("获得奖励: 【HEALTH_CHANCE】x" + _0x3dd9fc.obj.healthChance);
          }
          // 显示下一次奖励
          if(_0x3dd9fc.obj.nextReward) {
            this.log("下一次奖励: 【" + _0x3dd9fc.obj.nextReward + "】");
          }
          // 显示财神卡数量
          if(_0x3dd9fc.obj.richCardNum) {
            this.log("财神卡数量: " + _0x3dd9fc.obj.richCardNum);
          }
        }
  ...
  // 游戏主流程
  async jika2024_game_play() {
    try {
        let output = [];
        output.push("\n╭───────── 开始套财神 ─────────╮");
        
        // 查询游戏状态
        let gameStatus = await this.jika2024_game_indexInfo();
        if(!gameStatus) {
            this.log("❌ 查询游戏状态失败");
            return;
        }
        ...
        // 显示游戏基本信息
        output.push(`剩余游戏次数: ${gameStatus.surplusPushTime || 0}`);
        if(gameStatus.gotTimesOfToday !== undefined) {
            output.push(`今日已玩次数: ${gameStatus.gotTimesOfToday}`);
        }
        
        // 如果没有游戏次数，直接返回
        if(!gameStatus.surplusPushTime) {
            output.push("⚠️ 今日套财神次数已用完");
            output.push("╰────────────────────────────╯");
        ...
        let initOutput = [];
        initOutput.push("\n╭───────── 初始化游戏 ─────────╮");
        let gameInfo = await this.jika2024_game_init();
        if(!gameInfo) {
            this.log("❌ 初始化游戏失败");
            return;
        }
        ...
        // 显示游戏信息
        initOutput.push(`▎复活次数: ${gameInfo.resurrectTime}`);
        if(gameInfo.currentLevelAward?.length > 0) {
            let awards = gameInfo.currentLevelAward.map(award => 
                `${award.currency}x${award.amount}`
            );
            initOutput.push(`▎当前关卡奖励: ${awards.join(", ")}`);
        }
        
        // 获取真实的当前关卡
        let currentIndex = gameInfo.currentIndex || 1;
        output.push(`当前进度: 第${currentIndex}关`);
        
        // 输出状态
        this.log(output.join('\n'));
        this.log(initOutput.join('\n'));
        initOutput.push("╰────────────────────────────╯");
        
        // 从当前关卡开始继续游戏
        for(let level = currentIndex; level <= 15; level++) {
            await this.wait(1000 + Math.random() * 2000);
            
            let levelOutput = [];
            levelOutput.push(`\n╭─────────  第 ${level} 关  ─────────╮`);
            let result = await this.jika2024_game_win(level);
            
            if(result?.success) {
                levelOutput.push(result.message);
                levelOutput.push("╰────────────────────────────╯");
                this.log(levelOutput.join('\n'));
        ...
        console.log("游戏执行异常:", e);
    }
  }

  // 完成一关
  async jika2024_game_win(level) {
    try {
        const params = {
            levelIndex: level
        };
        ...
        if(result?.success) {
            let rewards = result.obj?.currentAwardList || [];
            ...
            if(rewards.length) {
                ...
                let rewardTexts = rewards.map(reward => 
                    `${rewardMap[reward.currency] || reward.currency}x${reward.amount}`
                );
                successInfo.push(`✅ 第 ${level} 关通关成功: ${rewardTexts.join(", ")}`);
            } else {
                successInfo.push(`✅ 第 ${level} 关通关成功`);
            }
            
            // 添加排名信息
            if(result.obj?.exceedPercent) {
                successInfo.push(`超越了${(result.obj.exceedPercent * 100).toFixed(1)}%的玩家，当前排名: ${result.obj.passRank}`);
            }
            
            return {
                success: true,
                nextLevel: result.obj?.nextLevelIndex > 0,
                ifFirstPassAllLevel: result.obj?.ifFirstPassAllLevel,
                message: successInfo.join('\n')
            };
        } else {
            let errorMsg = result?.errorMessage || (result ? JSON.stringify(result) : "无返回");
            return {
                success: false,
                message: `❌ 第 ${level} 关失败: ${errorMsg}`
            };
        }
    } catch(e) {
        console.log(e);
        return {
            success: false,
            message: `❌ 第 ${level} 关异常: ${e.message}`
        };
    }
}

  // 查询财神卡状态
  async jika2024_wealthStatus() {
    try {
        let status = await this.jika2024_checkWealthStatus();
        if(!status) return;
        
        // 显示招财神状态（无论是否有次数）
        let fortuneOutput = [];
        fortuneOutput.push("\n╭───────── 开始招财神─────────╮");
        fortuneOutput.push(`剩余招财神次数: ${status.wealthChance || 0}`);
        
        // 只有在有次数的时候才执行招财神
        if(status.wealthChance > 0) {
            for(let i = 1; i <= status.wealthChance; i++) {
                let result = await this.jika2024_fortuneWealth();
                if(result?.success) {
                    fortuneOutput.push(`🎯 第${i}次: 获得[${result.reward}${result.amount > 1 ? 'x'+result.amount : ''}]`);
                } else {
                    fortuneOutput.push(`❌ 第${i}次: ${result.errorMsg}`);
                    break;
                }
        ...
        fortuneOutput.push("╰────────────────────────────╯");
        this.log(fortuneOutput.join('\n'));
        
        // 重新获取最新状态
        status = await this.jika2024_checkWealthStatus();
        if(!status) return;
        
        // 显示最新的五路财神状态
        let output = [];
        output.push("\n╭───────── 五路财神状态 ─────────╮");
        output.push(`东财神: ${status.eastWealth || 0}张`);
        output.push(`南财神: ${status.southWealth || 0}张`);
        output.push(`西财神: ${status.westWealth || 0}张`);
        output.push(`北财神: ${status.northWealth || 0}张`);
        output.push(`中财神: ${status.middleWealth || 0}张`);
        output.push(""); // 空行分隔
        output.push(`万能卡: ${status.commonCard || 0}张`);
        output.push(`好运签: ${status.luckySign || 0}张`);
        output.push(`好运值: ${status.luckyValue || 0}点`);
        this.log(output.join('\n'));
        
    } catch(e) {
        console.log("五路财神执行异常:", e);
    }
  }

  // 添加一个只查询状态不输出日志的函数
  async jika2024_checkWealthStatus() {
    try {
        let options = {
            fn: "jika2024_wealthStatus",
            method: "post",
        ...
        if(result?.success && result.obj) {
            ...
            // 更新卡片数量
            const currencyMap = {
                'WEALTH_CHANCE': 'wealthChance',
                'EAST_WEALTH': 'eastWealth',
                'SOUTH_WEALTH': 'southWealth',
                'WEST_WEALTH': 'westWealth',
                'NORTH_WEALTH': 'northWealth',
                'MIDDLE_WEALTH': 'middleWealth',
                'COMMON_CARD': 'commonCard',
                'LUCKY_SIGN': 'luckySign',
                'LUCKY_VALUE': 'luckyValue',
                'LOVE_SIGN': 'loveSign'
            };
            
            for(let card of obj.currentAccountList) {
                if(currencyMap[card.currency]) {
                    status[currencyMap[card.currency]] = card.balance;
                }
            }
        ...
        console.log("查询财神状态异常:", e);
    }
    return null;
  }

  // 初始化游戏
  async jika2024_game_init() {
    try {
        let options = {
        fn: "jika2024_game_init",
        method: "post",
        url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024GameService~init",
        headers: {
                ...this.getSign(),
        ...
        // 直接打印完整的请求信息
       ("init请求:", JSON.stringify(options, null, 2));
        
        let response = await this.request(options);
        // 打印原始response
       ("init完整返回:", JSON.stringify(response, null, 2));
        ...
        if(!response.result) {
           ("❌ 返回数据缺少result字段:", response);
            return null;
        }
        
        if(!response.result.success) {
           ("❌ 请求失败:", response.result.errorMessage || JSON.stringify(response.result));
            return null;
        }
  ...
  // 抽财神卡
  async jika2024_drawRichCard() {
    try {
      let _0x3a5581 = {
        fn: "jika2024_drawRichCard",
        method: "post",
        url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024GameService~drawRichCard",
        headers: {
          ...this.getSign()
        },
        json: {}
      ...
      if (_0x3dd9fc?.["success"]) {
        if(_0x3dd9fc.obj?.richCardNum) {
          _0x1366a8.log("抽中财神卡，当前数量: " + _0x3dd9fc.obj.richCardNum);
        }
        return true;
      }
    } catch (_0x33e13c) {
  ...
  // 领取倒计时奖励
  async jika2024_getCountdownTimes() {
    try {
      let _0x3a5581 = {
        fn: "jika2024_getCountdownTimes",
        method: "post",
        url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024WealthCardService~getCountdownTimes",
        headers: {
          ...this.getSign()
        },
        json: {}
  ...
  // 使用财神卡抽奖
  async jika2024_fortuneWealth() {
    try {
        let options = {
        fn: "jika2024_fortuneWealth",
        method: "post",
        ...
        let { result } = await this.request(options);
        // 添加调试日志
        // this.log("招财神返回:", JSON.stringify(result, null, 2));
        ...
        if(result?.success) {
            ...
            let reward = result.obj?.receivedAccountList?.[0]; // 改用 receivedAccountList
            if(reward) {
                return {
                    success: true,
                    reward: rewardMap[reward.currency] || reward.currency,
        ...
        return {
            success: false,
            errorMsg: result?.errorMessage || '招财神失败'
        };
    } catch(e) {
        console.log("招财神异常:", e);
        return {
  ...
  // 查询游戏进度
  async jika2024_game_progress() {
    try {
      let _0x3a5581 = {
        fn: "jika2024_game_progress",
        method: "post",
        url: "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~yearEnd2024GameService~progress",
        headers: {
          ...this.getSign()
        },
        json: {}
      ...
      let { result: _0x3dd9fc } = await this.request(_0x3a5581);
      if (!_0x3dd9fc?.["success"]) {
        let errorMsg = _0x3dd9fc?.["errorMessage"] || "未知错误";
        _0x1366a8.log(`查询游戏进度失败: ${errorMsg}`);
        return null;
      }
      let progress = _0x3dd9fc.obj;
      // 显示进度信息
      if(progress.currentLevel) {
        _0x1366a8.log(`当前游戏进度: 第${progress.currentLevel}关`);
        _0x1366a8.log(`当前关卡配置: ${JSON.stringify(progress.levelConfig)}`);
      }
      return progress;
    } catch (_0x33e13c) {
      console.log(_0x33e13c);
  ...
  // 添加查询游戏状态函数
  async jika2024_game_indexInfo() {
    try {
        let options = {
        fn: "jika2024_game_indexInfo",
        method: "post",
        ...
        // 直接打印完整的请求信息
      ("indexInfo请求:", JSON.stringify(options, null, 2));
        
        let response = await this.request(options);
        // 打印原始response
        ("indexInfo完整返回:", JSON.stringify(response, null, 2));
...
!(async () => {
    ...
    // 最后统一推送所有账号的消息
    if(_0x1366a8.pushMsgList.length > 0) {
      const notify = {
        notify: true,
        pushMsg: _0x1366a8.pushMsgList.join('\n\n')
      };
      _0x1366a8.log("", notify);
    }
